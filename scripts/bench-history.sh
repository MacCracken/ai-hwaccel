...
# Build data: {benchmark: {timestamp: (low, est, high)}}
data = OrderedDict()
commits = {}
for r in rows:
    ts = r["timestamp"]
...
for i, ts in enumerate(pick):
    ...
    if len(pick) >= 3:
        if i == 0:
            ...
            labels.append(f"Previous (`{commits[ts]}`)")
    else:
        labels.append(f"{short} (`{commits[ts]}`)")
...
with open(md_file, "w") as f:
    ...
    if len(pick) >= 3:
        f.write(f"Tracking: `{commits[pick[0]]}` (baseline) → "
                f"`{commits[pick[1]]}` (previous) → "
                f"`{commits[pick[-1]]}` (current)\n\n")

    # Group by criterion group prefix
    groups = OrderedDict()
    for bench in data:
        parts = bench.split("/")
    ...
    for group, benches in groups.items():
        ...
        for bench in benches:
            ...
            for i, ts in enumerate(pick):
                ...
                if i == 0:
                    baseline_ns = est
                elif baseline_ns is not None:
                    cell += delta(baseline_ns, est)
                cells.append(cell)
